...
def build_tag_map(data, label="item"):
    tag_map = {}
    for name, fields in (data or {}).items():
        ...
        if isinstance(fields, dict):
            for key in [
                ...
                if val:
                    tags.add(f"{key}:{val}")
            if "trainerClass" in fields:
                tags.add("trainer")
            if "speciesName" in fields:
                tags.add("pokemon")
        tag_map[str(name)] = sorted(tags)
    return tag_map
...
class DarkLabel(tk.Label):
    def __init__(self, parent, **kw):
        super().__init__(
            parent, bg=kw.pop("bg", BG_DARK), fg=kw.pop("fg", FG_MAIN), **kw
        )
...
class SearchableList(tk.Frame):
    """Dark-themed searchable listbox with tags, sorting, and favorites support."""
    ...
    def __init__(self, parent, on_select=None, **kw):
        ...
        tk.Entry(
            ...
            highlightthickness=1,
            highlightbackground="#444466",
        ).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(4, 4), pady=4)

        toolbar = tk.Frame(self, bg=BG_MED)
        toolbar.pack(fill=tk.X, padx=4, pady=(0, 4))
        self._sort_var.trace_add("write", lambda *_: self._filter())
        ...
        tk.Checkbutton(
            toolbar,
            text="★ Favorites only",
            variable=self._show_favorites_var,
            bg=BG_MED,
            fg=FG_GOLD,
        ...
        self._tag_values = ["All tags"]
        self._tag_cb = ttk.Combobox(
            toolbar,
        ...
        self._tag_cb.pack(side=tk.LEFT, padx=(8, 0))
        self._tag_var.trace_add("write", lambda *_: self._filter())
    ...
    def _item_name(self, item):
        if isinstance(item, dict):
            return str(item.get("name") or item.get("id") or item.get("species") or item.get("move") or "")
        return str(item)

    def _item_tags(self, item):
        if isinstance(item, dict):
            tags = item.get("tags", [])
    ...
    def _filter(self, *_):
        ...
        favorites_only = self._show_favorites_var.get()
        ...
        for item in self._all:
            ...
            query_ok = not q or q in name.lower() or any(q in tag.lower() for tag in tags)
            if not query_ok:
                continue
            if favorites_only and name not in self._favorites:
                continue
            if tag_filter not in ("All tags", "") and tag_filter not in tags:
                continue
        ...
        for name in filtered:
            ...
            self._display_map[display] = name
            self._lb.insert(tk.END, display)
        self._count_var.set(f"{len(filtered)} / {len(self._all)}")

    def set_tags(self, tag_map):
        self._tag_map = {str(k): {str(v) for v in values} for k, values in (tag_map or {}).items()}
        tag_values = sorted({tag for values in self._tag_map.values() for tag in values})
        if tag_values:
            self._tag_values = ["All tags"] + tag_values
        else:
            self._tag_values = ["All tags"]
        if hasattr(self, "_tag_cb"):
            self._tag_cb["values"] = self._tag_values
            if self._tag_var.get() not in self._tag_values:
    ...
    def add_favorite(self, name):
        if not name:
            return
        self._favorites.add(str(name))
        state = load_editor_state()
        state["favorites"] = {"items": sorted(self._favorites)}
    ...
    def remove_favorite(self, name):
        if not name:
            return
        self._favorites.discard(str(name))
        state = load_editor_state()
        state["favorites"] = {"items": sorted(self._favorites)}
        save_editor_state(state)
        self._filter()
    ...
    def toggle_favorite(self, name):
        if str(name) in self._favorites:
    ...
    def get_selected(self):
        ...
        if not sel:
            return None
        display = self._lb.get(sel[0])
        return self._display_map.get(display)
    ...
    def select(self, name):
        name = str(name)
        display = self._display_name(name)
        values = list(self._lb.get(0, tk.END))
...
class TypeBadge(tk.Label):
    ...
    def set_type(self, type_name):
        ...
        if icon:
            ...
            badge = img_util.make_type_badge_image(
                type_name, self._size[0], self._size[1]
            )
...
class StatRow(tk.Frame):
    ...
    def __init__(self, parent, label, field, color, max_val=255):
        ...
        self._bar_canvas = tk.Canvas(
            self, height=14, bg="#2a2a3e", highlightthickness=0, bd=0
        )
    ...
    def _redraw_bar(self, *_):
        ...
        self._bar_canvas.create_rectangle(
            0, 2, fill_w, h - 2, fill=bar_color, outline=""
        )
...
class PokemonTab(DarkFrame):
    ...
    def _build_ui(self):
        ...
        tk.Label(
            left, text="POKÉMON", bg=BG_MED, fg=FG_ACCENT, font=("Segoe UI", 11, "bold")
        ).pack(pady=(10, 0))
        ...
        tk.Label(
            row1, text="Species Name:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)
        ).pack(side=tk.LEFT)
        self._name_var = tk.StringVar()
        DarkEntry(
            row1, textvariable=self._name_var, width=18, font=("Segoe UI", 12, "bold")
        ).pack(side=tk.LEFT, padx=8)

        row2 = tk.Frame(info_card, bg=BG_CARD)
        row2.pack(fill=tk.X, padx=8, pady=4)
        tk.Label(
            row2, text="Category:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)
        ).pack(side=tk.LEFT)
        self._cat_var = tk.StringVar()
        DarkEntry(row2, textvariable=self._cat_var, width=14).pack(side=tk.LEFT, padx=8)
        tk.Label(
            row2, text="Pokémon", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)
        ).pack(side=tk.LEFT)
        ...
        tk.Label(
            type_row, text="Types:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)
        ).pack(side=tk.LEFT)
        ...
        self._type1_cb = ttk.Combobox(
            type_row, textvariable=self._type1_var, width=11, state="readonly"
        )
        self._type1_cb.pack(side=tk.LEFT, padx=(12, 4))
        self._type1_cb.bind(
            "<<ComboboxSelected>>",
            lambda e: (self._type1_badge.set_type(self._type1_var.get()),),
        )
        self._type2_cb = ttk.Combobox(
            type_row, textvariable=self._type2_var, width=11, state="readonly"
        )
        self._type2_cb.pack(side=tk.LEFT, padx=4)
        self._type2_cb.bind(
            "<<ComboboxSelected>>",
            lambda e: (self._type2_badge.set_type(self._type2_var.get()),),
        )
        ...
        for i, lbl in enumerate(["Ability 1", "Ability 2", "Hidden"]):
            ...
            tk.Label(col, text=lbl, bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 8)).pack(
                anchor="w"
            )
            cb = ttk.Combobox(
                col, textvariable=self._ab_vars[i], width=18, state="readonly"
            )
            cb.pack(fill=tk.X)
        ...
        for col_idx, (label, field, tip) in enumerate(
            ...
            tk.Label(
                cell, text=label, bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 8)
            ).pack(anchor="w")
            var = tk.StringVar()
            self._other_vars[field] = var
            DarkEntry(cell, textvariable=var, width=8, justify="center").pack(fill=tk.X)
            tk.Label(cell, text=tip, bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 7)).pack(
                anchor="w"
            )
    ...
    def _update_bst(self):
        try:
            total = sum(int(self._stat_rows[f].get_value() or 0) for f, *_ in STAT_DEFS)
    ...
    def _load_sprite(self, species_name, shiny=False):
        ...
        if sprite:
            ...
            if icon:
                ...
                self._sprite_lbl.configure(
                    image="", text=f"No sprite\n{species_name}", width=12, height=6
                )
                self._img_ref = None
...
class MovesTab(DarkFrame):
    ...
    def _build_ui(self):
        ...
        paned.add(left, minsize=180)
        tk.Label(
            left, text="MOVES", bg=BG_MED, fg=FG_ACCENT, font=("Segoe UI", 11, "bold")
        ).pack(pady=(10, 0))
        ...
        tk.Label(row1, text="Name:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)).pack(
            side=tk.LEFT
        )
        self._name_var = tk.StringVar()
        DarkEntry(
            row1, textvariable=self._name_var, width=20, font=("Segoe UI", 12, "bold")
        ).pack(side=tk.LEFT, padx=8)

        row2 = tk.Frame(header, bg=BG_CARD)
        row2.pack(fill=tk.X, padx=8, pady=(0, 8))

        tk.Label(row2, text="Type:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)).pack(
            side=tk.LEFT
        )
        ...
        self._type_cb = ttk.Combobox(
            row2, textvariable=self._type_var, width=12, state="readonly"
        )
        self._type_cb.pack(side=tk.LEFT, padx=(4, 20))
        self._type_cb.bind(
            "<<ComboboxSelected>>",
            lambda e: self._type_badge.set_type(self._type_var.get()),
        )
        ...
        for i, (label, field, tip, color) in enumerate(stat_data):
            ...
            tk.Label(
                cell, text=label, bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)
            ).pack(anchor="w")
            var = tk.StringVar()
        ...
        tk.Label(
            pb_frame, text="Power:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)
        ).pack(side=tk.LEFT)
        self._power_canvas = tk.Canvas(
            pb_frame, height=12, bg="#2a2a3e", highlightthickness=0, bd=0
        )
    ...
    def set_data(self, data, types):
        ...
        self._list.set_items(sorted(data.keys()), tags=build_tag_map(data, "move"))
...
class ItemsTab(DarkFrame):
    ...
    def _build_ui(self):
        ...
        paned.add(left, minsize=180)
        tk.Label(
            left, text="ITEMS", bg=BG_MED, fg=FG_ACCENT, font=("Segoe UI", 11, "bold")
        ).pack(pady=(10, 0))
        ...
        tk.Label(
            name_col, text="Item Name:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)
        ).pack(anchor="w")
        self._name_var = tk.StringVar()
        DarkEntry(
            name_col,
        ...
        tk.Label(
            name_col, text="Constant:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 8)
        ).pack(anchor="w")
        self._const_lbl = tk.Label(
            name_col, text="", bg=BG_CARD, fg=FG_DIM, font=("Consolas", 9)
        )
    ...
    def set_data(self, data):
        self._data = data
        self._list.set_items(sorted(data.keys()), tags=build_tag_map(data, "item"))
...
class TrainersTab(DarkFrame):
    ...
    def _build_ui(self):
        ...
        # Sprite
        self._trainer_sprite = tk.Label(
            header_row, bg=BG_CARD, text="👤", font=("Segoe UI", 32), width=4, height=4
        )
        ...
        for label, field in [("Gender", "gender"), ("Battle Type", "battleType")]:
            tk.Label(
                extra_row, text=f"{label}:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)
            ).pack(side=tk.LEFT)
            var = tk.StringVar()
            self._trainer_vars[field] = var
            DarkEntry(extra_row, textvariable=var, width=14).pack(
        ...
        self._party_tree = ttk.Treeview(
            party_card, columns=cols, show="headings", height=8, style="Dark.Treeview"
        )
        for col in cols:
            self._party_tree.heading(col, text=col)
            self._party_tree.column(col, width=120)

        party_scroll = ttk.Scrollbar(
            party_card, orient=tk.VERTICAL, command=self._party_tree.yview
        )
    ...
    def _load_trainer(self, name):
        ...
        threading.Thread(
            target=self._bg_load_trainer_sprite, args=(trainer_pic,), daemon=True
        ).start()
        ...
        for i, lbl in enumerate(self._party_icon_labels):
            if i < len(party):
                ...
                threading.Thread(
                    target=self._bg_load_party_icon, args=(i, species), daemon=True
                ).start()
            else:
...
class TypesTab(DarkFrame):
    ...
    def _build_ui(self):
        ...
        tk.Label(
            header,
            text="Row = Attacker  ·  Column = Defender",
            bg=BG_DARK,
            fg=FG_DIM,
            font=("Segoe UI", 9),
        ...
        hscroll = ttk.Scrollbar(
            canvas_container, orient=tk.HORIZONTAL, command=self._canvas.xview
        )
        vscroll = ttk.Scrollbar(
            canvas_container, orient=tk.VERTICAL, command=self._canvas.yview
        )
    ...
    def _draw_chart(self):
        ...
        for j, t in enumerate(types):
            ...
            self._canvas.create_rectangle(
                x, 0, x + cell, header_h, fill=color, outline=BG_DARK, width=1
            )
        ...
        for i, t in enumerate(types):
            ...
            color = TYPE_COLORS.get(t, "#555")
            self._canvas.create_rectangle(
                0, y, header_w, y + cell, fill=color, outline=BG_DARK, width=1
            )
            name = t[:7]
            self._canvas.create_text(
                header_w // 2,
        ...
        for i, attacker in enumerate(types):
            for j, defender in enumerate(types):
                ...
                self._canvas.create_rectangle(
                    x + 1, y + 1, x + cell - 1, y + cell - 1, fill=color, outline=""
                )
...
class AbilitiesTab(DarkFrame):
    ...
    def _build_ui(self):
        ...
        tk.Label(inner, text="Name:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)).pack(
            anchor="w"
        )
        self._name_lbl = tk.Label(
            inner, text="", bg=BG_CARD, fg=FG_MAIN, font=("Segoe UI", 16, "bold")
        )
        self._name_lbl.pack(anchor="w", pady=4)

        tk.Label(
            inner, text="Constant:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 8)
        ).pack(anchor="w")
        self._const_lbl = tk.Label(
            inner, text="", bg=BG_CARD, fg=FG_DIM, font=("Consolas", 9)
        )
        ...
        tk.Label(
            rating_row, text="AI Rating:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 9)
        ).pack(side=tk.LEFT)
        self._rating_lbl = tk.Label(
            rating_row, text="", bg=BG_CARD, fg=FG_GOLD, font=("Segoe UI", 11, "bold")
        )
        ...
        self._users_tree = ttk.Treeview(
            ...
            show="headings",
            height=10,
            style="Ability.Treeview",
        )
        for col in cols:
            self._users_tree.heading(col, text=col)
            self._users_tree.column(col, width=120)
        ab_scroll = ttk.Scrollbar(
            users_card, orient=tk.VERTICAL, command=self._users_tree.yview
        )
    ...
    def set_data(self, abilities_data, species_data):
        ...
        self._list.set_items(sorted(abilities_data.keys()), tags=build_tag_map(abilities_data, "ability"))
...
class BuildTab(DarkFrame):
    ...
    def _build_ui(self):
        ...
        tk.Label(
            self,
            text="Compile your modifications into a playable GBA ROM",
            bg=BG_DARK,
            fg=FG_DIM,
            font=("Segoe UI", 10),
        ...
        tk.Label(
            opts_inner, text="Target:", bg=BG_CARD, fg=FG_DIM, font=("Segoe UI", 10)
        ).pack(side=tk.LEFT)
        ...
        ttk.Combobox(
            ...
            width=14,
            state="readonly",
            values=["(default)", "debug", "release", "check", "clean"],
        ).pack(side=tk.LEFT, padx=8)
        ...
        DarkEntry(
            opts_inner, textvariable=self._jobs_var, width=5, justify="center"
        ).pack(side=tk.LEFT, padx=8)
    ...
    def _start_build(self, mode):
        ...
        threading.Thread(
            target=self._do_build, args=(script, args), daemon=True
        ).start()
    ...
    def _do_build(self, script, args):
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                self.after(
                    0, self._status_var.set, f"❌ Build failed (exit {proc.returncode})"
                )
...
class App(tk.Tk):
    ...
    def _setup_styles(self):
        ...
        style.configure(
            "TScrollbar", background=BG_MED, troughcolor=BG_DARK, arrowcolor=FG_DIM
        )
    ...
    def _build_ui(self):
        ...
        tk.Label(
            header, text="🎮", bg="#0d0d1a", fg=FG_ACCENT, font=("Segoe UI", 20)
        ).pack(side=tk.LEFT, padx=(12, 4), pady=8)
        tk.Label(
            header,
            text="pokeemerald-expansion Editor",
            bg="#0d0d1a",
            fg=FG_MAIN,
            font=("Segoe UI", 14, "bold"),
        ...
        self._loading_lbl = tk.Label(
            header, text="Loading data…", bg="#0d0d1a", fg=FG_DIM, font=("Segoe UI", 9)
        )
    ...
    def _load_data(self):
        ...
        for name, fn in steps:
            self.after(0, self._loading_lbl.config, {"text": f"Loading {name}…"})
            try:
                loaded[name] = fn()
            except Exception as e:
                loaded[name] = {}
    ...
    def _apply_data(self, loaded, types, abilities_list, effectiveness):
        ...
        counts = (
            f"{len(species)} species, {len(moves)} moves, "
